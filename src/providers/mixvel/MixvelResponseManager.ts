...
type MixvelMessage = Record<
  string,
  { $: string[]; Response?: unknown[]; Error?: MixvelError[] }[]
>;
type MixvelError = {
  ErrorType?: string;
  CanRetry?: string;
...
type MixvelCompleteResponse = Record<
  string,
  { Body?: { AppData?: MixvelMessage[]; Error?: MixvelError[] }[] }
>;
...
class MixvelResponseMapper implements IResponseMapper {
  ...
  public map(
    completeResponseObject: MixvelCompleteResponse
  ): MixvelResponseError | MixvelResponseMessage {
    let appData;
...
export class MixvelResponseManager extends AbstractResponseManager {
  ...
  constructor(
    ...
    super(
      conversionStrategy,
      new MixvelResponseMapper(
        MixvelResponseManager.rootNodeName,
        responseTypes
      ),
      allowedDataLists
    );
  ...
  async getResponse(
    rawXML: string
  ): Promise<MixvelResponseMessage | MixvelResponseError> {
    return this.convert(rawXML).then((responseObject) => {
      ...
      if (
        responseObject[MixvelResponseManager.rootNodeName] === undefined ||
        responseObject[MixvelResponseManager.rootNodeName]?.Body === undefined
      ) {
        return Promise.reject(
          new ResponseParsingError("Invalid response format")
        );
      }
  ...
  createDataList(
    ...
    dataListSource: Record<string, unknown>[]
  ): IDataList {
