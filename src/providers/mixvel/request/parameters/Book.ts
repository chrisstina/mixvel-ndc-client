...
export class MixvelPassenger extends Passenger {
  ...
  constructor(
    ...
    if (loyaltyInfo) {
      this.loyaltyInfo = new MixvelLoyaltyInfo(
        loyaltyInfo.code,
        loyaltyInfo.carrier,
        loyaltyInfo.opts
      );
    }
    this.contacts = new MixvelContact(
    ...
    if (ssrRemarks) {
      ...
      ssrRemarks.forEach((remark) => {
        this.ssrRemarks?.push(
          new MixvelSSRRemark(remark.type, remark.text, remark.action, id)
        );
      });
    }
    if (subsidyData) {
      this.subsidyData = new MixvelSubsidyInformation(
        subsidyData.program,
        subsidyData.type
      );
    }
