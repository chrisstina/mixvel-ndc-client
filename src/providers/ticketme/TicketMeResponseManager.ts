...
type TicketMeCompleteResponse<Namespace extends string> = Record<
  string,
  Record<
    | "$"
    | `${Namespace}:Success`
...
class TicketMeResponseMapper implements IResponseMapper {
  ...
  public map(
    completeResponseObject: Partial<TicketMeCompleteResponse<CurrentNamespace>>
  ): TicketMeResponseError | TicketMeResponseMessage {
