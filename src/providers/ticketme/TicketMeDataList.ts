...
export class TicketMeDataList extends DataList {
    protected constructor(entityName: string, items: Array<any>, dataListEntityName?: string) {
        ...
        try {
            this.list = items[0][`${DEFAULT_NAMESPACE}:${entityName}List`][0][`${DEFAULT_NAMESPACE}:${dataListEntityName || entityName}`]
        } catch (e: any) {
    ...
    public static create(dataListTitle: string, dataListSource: Array<any>, dataListEntityName?: string) {
