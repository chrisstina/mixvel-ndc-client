...
class PriceRequestUnitTest {
  @test "Create price RQ several offer items"() {
    ...
    expect(rq).to.contain(
      '<Offer Owner="KW" OfferID="SOME_OFFER" ResponseID="SOME_RESONSE_ID">'
    );
  ...
  @test "Validate price RQ params with specific provider rules"() {
    ...
    expect(rq.isFailure).to.be.true;
    expect(rq.error).to.contain(
      "RequestValidationError: offerOwner must be longer than or equal to 1 characters"
    );
  }
}
