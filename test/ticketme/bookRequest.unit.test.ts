...
class BookRequestUnitTest {
    @test 'Create TicketMe book RQ for 1ADT RT'() {
        ...
        expect(rq).to.contain('<Offer Owner="KW" OfferID="SOME-OFFER" ResponseID="123">')
        expect(rq).to.contain('OfferItemID="OFFER-1"')
        expect(rq).to.contain('<PassengerRefs>pax1')
        expect(rq).to.contain('PassengerID="pax1">\n' +
            '\t\t\t\t\t<PTC>ADT</PTC>')
        expect(rq).to.contain('<PassengerRefs>pax1')
        expect(rq).to.contain('<EmailAddressValue>test@test.test</EmailAddressValue>')
        expect(rq).to.contain('<PhoneNumber>89112311441</PhoneNumber>')
        expect(rq).to.contain('<NameTitle>Mr</NameTitle>\n' +
    ...
    @test 'Create Ticketme book RQ for 2ADT 1CHD 1INF RT'() {
        ...
        const params: BookProps = {
            offer: {
                offerId: 'SOME-OFFER',
                offerItems: [{offerItemId: 'OFFER-1', paxs: "pax1 pax2"}, {offerItemId: 'OFFER-2', paxs: "pax3"}, {offerItemId: 'OFFER-3', paxs: "pax4"}],
                offerOwner: "KW",
                responseId: "123"
            },
            passengers: [
